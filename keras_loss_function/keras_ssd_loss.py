...
class SSDLoss:
    ...
    def smooth_L1_loss(self, y_true, y_pred):
        ...
        return tf.math.reduce_sum(l1_loss, axis=-1)
    ...
    def log_loss(self, y_true, y_pred):
        ...
        y_pred = tf.math.maximum(y_pred, 1e-15)
        # Compute the log loss
        log_loss = -tf.math.reduce_sum(y_true * tf.math.log(y_pred), axis=-1)
        return log_loss
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        '''
        if isinstance(self.neg_pos_ratio, (float, int)):
            self.neg_pos_ratio = tf.constant(self.neg_pos_ratio)
        if isinstance(self.n_neg_min, (float, int)):
        ...
        positives = tf.cast(tf.math.reduce_max(y_true[:, :, 1:-12], axis=-1), dtype=tf.float32) # Tensor of shape (batch_size, n_boxes)
        ...
        pos_class_loss = tf.math.reduce_sum(classification_loss * positives, axis=-1) # Tensor of shape (batch_size,)
        ...
        loc_loss = tf.math.reduce_sum(localization_loss * positives, axis=-1) # Tensor of shape (batch_size,)

        # 4: Compute the total loss.

        total_loss = (class_loss + self.alpha * loc_loss) / tf.math.maximum(1.0, n_positive) # In case `n_positive == 0`
