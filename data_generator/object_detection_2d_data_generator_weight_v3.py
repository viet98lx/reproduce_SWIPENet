...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def parse_xml(self,
        ...
        for images_dir, image_set_filename, annotations_dir in zip(images_dirs, image_set_filenames, annotations_dirs):
            ...
            if verbose:
                ...
                it = image_ids
            # Read weights for each image here
            with open(sample_weights_dirs) as f:
    ...
    def parse_yolo_text(self,
        ...
        self.sample_weights = []
        self.eval_neutral = []
        class_dict = {1: 'seaurchin', 2: 'starfish', 3: 'seacucumber', 4: 'scallop', 5: 'waterweeds'}
        if not annotations_dirs:
            self.labels = None
            self.eval_neutral = None
            annotations_dirs = [None] * len(images_dirs)
        ...
        for images_dir, image_set_filename, annotations_dir in zip(images_dirs, image_set_filenames, annotations_dirs):
            ...
            for image_id in it:
                ...
                if not annotations_dirs is None:
                    ...
                    with open(os.path.join(annotations_dir, image_id + '.txt')) as f:
                        objects = f.readlines()
                    ...
                    for obj in objects:
                        try:
                            class_id, x_min, y_min, w, h = re.split(" ", obj.strip())
                            ...
                            w = float(w)
                            h = float(h)
                            x_min, y_min, w, h = pbx.convert_bbox((x_min, y_min, w, h), from_type="yolo",
                                                                  to_type="coco", image_size=(W, H))
                            ...
                            strset = str(image_set_filenames)
                            if 'trainval' in strset:  # parse the trainval set
                                boxstr = image_id + ' ' + str(xmin) + ' ' + str(ymin) + ' ' + str(xmax) + ' ' + str(
                                    ymax)
                                index = gtboxlist.index(boxstr)
                                weight = float(gtweightlist[index])
                                box.append(weight)
                            ...
                            with open("Image_err.txt", 'a') as f:
                                print(traceback.format_exc())
                                f.write(
                                    image_id + ' ' + str(xmin) + ' ' + str(ymin) + ' ' + str(xmax) + ' ' + str(ymax))
                                f.write("\n")
                                f.close()
                    self.labels.append(boxes)
                    self.eval_neutral.append(eval_neutr)
    ...
    def parse_weight(self,
        ...
        # create the weights.txt file and initialize the weights of each training sample

        weightfid = open(sample_weights_dirs, 'w')
        for images_dir, image_set_filename, annotations_dir in zip(images_dirs, image_set_filenames, annotations_dirs):
            # Read the image set file that so that we know all the IDs of all the images to be included in the dataset.
            with open(image_set_filename) as f:
                image_ids = [line.strip() for line in f]  # Note: These are strings, not integers.
                ...
                if not annotations_dirs is None:
                    ...
                    for obj in objects:
                        try:
                            ...
                            xmax = x_min + w
                            ymax = y_min + h
                            # initialize the weight of each training sample as 1
                            if not sample_weights_dirs is None:
                                weightstr = image_id + ' ' + str(xmin) + ' ' + str(ymin) + ' ' + str(xmax) + ' ' + str(
                                    ymax) + ',' + str(1)
                                weightfid.writelines(weightstr + '\n')
                        except:
                            with open("Image_err.txt", 'a') as f:
                                print(traceback.format_exc())
                                f.write(
    ...
    def __generate_data(self, index, shuffle=True,
                 transformations=[],
                 label_encoder=None,
                 returns={'processed_images', 'encoded_labels'},
                 keep_images_without_gt=False,
        ...
        # 3) Else, if we have neither of the above, we'll have to load the individual image
        #    files from disk.
        batch_indices = self.dataset_indices[index:index + self.batch_size]
        if not (self.images is None):
            for i in batch_indices:
                batch_X.append(self.images[i])
            if not (self.filenames is None):
                batch_filenames = self.filenames[index:index + self.batch_size]
            else:
                batch_filenames = None
        elif not (self.hdf5_dataset is None):
            for i in batch_indices:
        ...
        # Get the labels for this batch (if there are any).
        if not (self.labels is None):
            batch_y = deepcopy(self.labels[index:index + self.batch_size])
        else:
            batch_y = None

        if not (self.eval_neutral is None):
            batch_eval_neutral = self.eval_neutral[index:index + self.batch_size]
        else:
            batch_eval_neutral = None

        # Get the image IDs for this batch (if there are any).
        if not (self.image_ids is None):
            batch_image_ids = self.image_ids[index:index + self.batch_size]
        else:
            batch_image_ids = None
    ...
    def __getitem__(self, index):
        if self.mode == 'training':
            return self.__generate_data(index, shuffle=True,
            ...
            return self.__generate_data(index, shuffle=False,
                 transformations=self.transformations,
                 label_encoder=self.label_encoder,
                 returns={'processed_images',
                          'encoded_labels'},
